...
if __name__ == "__main__":
    ...
    demos = np.concatenate(
        (demos, np.ones((demos.shape[0], demos.shape[1], 1))), axis=2
    )
    ...
    input_manifold.transport = types.MethodType(
        sphere_parallel_transport, input_manifold
    )
    # Output manifold
    output_manifold = Sphere(output_dim)
    # Replace transport operation by parallel transport
    output_manifold.transport = types.MethodType(
        sphere_parallel_transport, output_manifold
    )
    ...
    dt = 1
    for i in range(199):
        x_dot, sigma_gmr, H = manifold_gmr(
            new_x.reshape(1, -1), manifold, gmm_means, gmm_covariances, gmm_priors
        )
        new_x = x_dot[0]
    ...
    for k in range(nb_clusters):
        plot_gaussian_mesh_on_tangent_plane(
            gmm_means[k, 0], gmm_covariances[k, :3, :3], color=(1, 0, 0)
        )

    for k in range(nb_clusters):
        plot_gaussian_mesh_on_tangent_plane(
            gmm_means[k, 1], gmm_covariances[k, 3:, 3:], color=(0, 1, 0)
        )
