...
if __name__ == "__main__":
    ...
    # Data
    X = dt * np.concatenate(
        [np.arange(demosE[i].shape[0])[:, None] for i in range(nb_samples)]
    )
    ...
    for p in range(nb_samples):
        ...
        plt.plot(
            Y1[p * nb_data : (p + 1) * nb_data, 0],
            Y1[p * nb_data : (p + 1) * nb_data, 1],
            ".",
            markersize=12,
            color=(0.0, 0.0, 0.0),
        )
    ...
    for k in range(nb_clusters):
        ...
        plot_gaussian_covariance(
            ax,
            gmm_means[k, 1],
            gmm_covariances[k][out1_idx][:, out1_idx],
            color=(0.5, 0, 0.2),
        )
    ...
    for k in range(nb_clusters):
        plot_gaussian_mesh_on_tangent_plane(
            gmm_means[k, 2],
            gmm_covariances[k][out2_idx][:, out2_idx],
            color=(0.5, 0, 0.2),
        )
    ...
    for n in range(nb_data + nb_data_sup):
        ...
        plot_gaussian_mesh_on_tangent_plane(
            mu_gmr[n, 2:], sigma_gmr[n][2:][:, 2:], color=(0.20, 0.54, 0.93)
        )
        # Plot mean only
