...
class TaskDemoEvaluator(object):
    ...
    def evaluate(self, ds, dataset, max_steps, sampling_dt, render=False, record=False):
        ...
        if record:
            ...
            status = None
            if self.cfg.wandb:
                wandb.log({f"{self.task_name} {status} {self.env.record_count}":wandb.Video(video_path, fps=30, format="gif")})
        ...
        if self.cfg.wandb:
            ...
            wandb.log({'task':self.task_name, 'accuracy': acc*100, \
    ...
    def run(self):
        ...
        ds_model_dirs = [os.path.join(self.cfg.skills_dir, self.cfg.state_type, t, ds[idx].name) for idx, t in enumerate(self.task)]
        ...
        self.logger.info(f'Evaluating DS of skills in {self.task_name} task with {self.cfg.state_type} input on CALVIN environment')
        # Create and load models to evaluate
        # Evaluate by simulating in the CALVIN environment
        acc, avg_return, avg_len = self.evaluate(ds, dataset, max_steps=self.cfg.max_steps, \
