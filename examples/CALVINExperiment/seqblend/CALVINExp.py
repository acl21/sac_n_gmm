...
class CALVINExp:
    ...
    def get_taskspace_training_data(self, fname="training.npy"):
        ...
        if self.demo_log is None:
            ...
            self.logger.info(
                f"Loaded task demo data from {os.path.join(self.demos_dir, task_dir, fname)}"
            )
        ...
        for i in range(len(self.skill_list)):
            desired_value = np.stack(
                [self.skill_fncs[i](X[t, :]) for t in np.arange(0, X.shape[0])]
            )
        ...
        self.logger.info("Returning task space training data!")
    ...
    def test_policy(self, policy, cfg, env):
        ...
        if self.demo_log is None:
            task_dir = "_".join(self.skill_names)
            self.demo_log = np.load(
                os.path.join(self.demos_dir, task_dir, fname="training.npy")
            )
        ...
        while np.linalg.norm(current_state - x0[:3]) >= error_margin:
            ...
            if count >= 200:
                ...
                self.logger.info(
                    x0[:3], current_state, np.linalg.norm(current_state - x0[:3])
                )
                break
        ...
        for i in range(timesteps):
            ...
            [
                self.skill_list[s].update_desired_value(self.skill_fncs[s](xt[:3]))
                for s in range(len(self.skill_list))
            ]
            ...
            dxt = np.hstack(
                (
                    self.skill_list[0].predict_dx(xt[:3]),
                    self.skill_list[1].predict_dx(xt[:3]),
                )
            )
            ...
            temp = np.append(
                xt_weighted, np.append(self.skill_list[0].dataset.fixed_ori, -1)
            )
            action = env.prepare_action(temp, type="abs")
            observation, reward, done, info = env.step(action)
            xt = np.hstack(
                (observation[start_idx:end_idx], observation[start_idx:end_idx])
            )
