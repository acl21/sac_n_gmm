...
class CALVINSeqBlend(object):
    ...
    def __init__(self, cfg, env):
        ...
        self.skill_dim = sum(
            [
                self.exp.skill_list[i].dataset.X.numpy().shape[-1]
                for i in range(len(self.exp.skill_list))
            ]
        )
        ...
        if self.cfg.policy_path is not None:
            if (
                os.path.exists(self.cfg.policy_path)
                and not self.cfg.update_policy
                and not self.cfg.eval_policy
            ):
    ...
    def run(self):
        ...
        batch_skill = CALVINSkillComplex(
            self.skill_dim, self.skill_dim, self.exp.skill_list, skill_cluster_idx=None
        )
        ...
        policy_config = {"dim": self.skill_dim, "fdim": 1, "skill": batch_skill}
