...
def gripper_experiment(options):
    ...
    # Create robot
    robot = Gripper2D(
        nb_dofs_arm=nb_dofs_arm,
        nb_dofs_finger=nb_dofs_finger,
        arm_link_length=arm_link_length,
    ...
    pick_pos_skill = TaskspaceSkillBatch(
        ts_pos,
        name="pick",
        compute_fct_batch=robot.compute_ts_arm_fct,
        compute_jacobian_fct_batch=robot.compute_ts_jacobian_arm_fct,
        config_idx=robot.arm_idx,
        state_idx=[0, 1, 2, 3],
    )

    ts_pos = np.array([[10, 0, 1, 0]])
    place_pos_skill = TaskspaceSkillBatch(
        ts_pos,
        name="place",
        compute_fct_batch=robot.compute_ts_arm_fct,
        compute_jacobian_fct_batch=robot.compute_ts_jacobian_arm_fct,
    ...
    close_gripper_skill = JointPositionSkillBatch(
        qclose, name="close", config_idx=robot.gripper_idx, state_idx=[4, 5, 6, 7, 8, 9]
    )

    qopen = np.array(
        [np.pi / 2, -np.pi / 2, -np.pi / 3, -np.pi / 2, np.pi / 2, np.pi / 3]
    )
    open_gripper_skill = JointPositionSkillBatch(
        qopen, name="open", config_idx=robot.gripper_idx, state_idx=[4, 5, 6, 7, 8, 9]
    )
    ...
    for i in range(num_data_file):
        ...
        skill_fcns = [
            create_ds_skill(
                "../../" + pick_clf, "../../" + pick_ds, pick_ori, pick_pos, speed=0.15
            ),
            create_ds_skill(
                "../../" + place_clf,
                "../../" + place_ds,
        ...
        skill_list, xt_d, dxt_d, desired_, ctrl_idx = exp.get_taskspace_training_data(
            fname=data_file_demo
        )
        batch_skill = SkillComplex(
            skill_dim, nb_dofs, skill_list, skill_cluster_idx=skill_cluster_idx
        )
    ...
    policy_config = {"dim": skill_dim, "fdim": 1, "skill": batch_skill}
    policy_type = getattr(policy_classes, options.policy_name)
    num_epochs_per_demo_pass = int(
        MAX_EPOCH / num_demos / 10
    )  # 10 passes through each demo
    ...
    if not os.path.exists(model_fpath):
        ...
        if policy_type == policy_classes.SkillFullyWeightedPolicy:
            diag_policy = policy_classes.SkillDiagonalWeightedPolicy(
                **policy_config, **constraints
            )
            ...
            while epochs < MAX_EPOCH:
                ...
                for i in range(num_demos):
                    ...
                    diag_policy = policy_classes.train_policy(
                        diag_policy,
                        dataloader,
        ...
        while epochs < MAX_EPOCH:
            ...
            for i in range(num_demos):
                ...
                policy = policy_classes.train_policy(
                    policy,
                    dataloader,
                    loss_weights=loss_weights,
                    model_path=model_fpath,
