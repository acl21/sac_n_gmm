...
def visualize_3d_gmm(points, w, mu, stdev, skill, export_dir, export_type="gif"):
    ...
    for i in range(n_gaussians):
        idx = range(i * N, (i + 1) * N)
        axes.scatter(
            points[idx, 0], points[idx, 1], points[idx, 2], alpha=0.3, c=[[0, 0, 0]]
        )
    ...
    if export_type == "gif":
        ...
        anim = animation.FuncAnimation(
            fig, animate, init_func=init, frames=180, interval=30, blit=True
        )
        # Save
...
# Source: https://github.com/sitzikbs/gmm_tutorial/
def plot_sphere3d(
    w=0, c=[0, 0, 0], r=[1, 1, 1], subdev=10, ax=None, sigma_multiplier=3
):
    """
