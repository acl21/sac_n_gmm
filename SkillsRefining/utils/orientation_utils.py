...
def sphere_logarithmic_map_batch(x, x0):
    ...
    distance = torch.acos(
        torch.clamp(torch.matmul(torch.transpose(x0, 1, 2), x), -1.0, 1.0)
    )
    ...
    u = torch.where(
        distance > 1e-16,
        (x - x0 * torch.cos(distance)) * distance / torch.sin(distance),
...
def sphere_derivative_logarithmic_map(x: np.ndarray, x0: np.ndarray) -> np.ndarray:
    ...
    if distance < 1e-6:
        ...
        derivative = distance * (id_matrix - np.dot(x0, x0.T)) * (
            id_matrix / norm_projection
            - np.dot(projection, projection.T) / norm_projection**3
        ) - np.dot(x0, (projection / norm_projection).T) / np.sqrt(
            1.0 - np.cos(distance) ** 2
        )
...
def compute_analytical_orientation_jacobian_sphere_batch(
    xo, orientation_jacobian, pos_dim=2
):
    """
